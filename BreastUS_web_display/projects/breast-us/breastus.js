/* Breast US page interactions: i18n + scrollspy + theme + links */
(function () {
  const $ = (sel, root = document) => root.querySelector(sel);
  const $$ = (sel, root = document) => Array.from(root.querySelectorAll(sel));

  const i18n = window.BREASTUS_I18N;
  if (!i18n) return;
  ...
  function setLang(nextLang, persist = true) {
    ...
    setMeta(state.lang);
    render();
    if (persist) localStorage.setItem("breastus_lang", state.lang);
  }
  ...
  function wire() {
    ...
    const savedLang = localStorage.getItem("breastus_lang");
